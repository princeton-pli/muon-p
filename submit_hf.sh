#!/usr/bin/env bash
# Simple submitter for Muon HF training sweeps.
...
TRAIN_SCRIPT="${TRAIN_SCRIPT:-$REPO_ROOT/run_hf.sh}"
SWEEP_PARAMS_YAML="${SWEEP_PARAMS_YAML:-$REPO_ROOT/config/sweep_params_hf.yaml}"
...
_process_yaml_run() {
  ...
  while [ "$i" -le "$#" ]; do
    ...
    # Export parameter as environment variable (lowercase for run_hf.sh)
    ...
    case "$lower_name" in
      ...
      max_steps) _max_steps="$param_value" ;;
  ...
  # Generate run_name in the same format as run_hf.sh
  local exponent_clean=$(echo "${_muon_exponent}" | tr '/' '-')
  local model_slug=$(echo "${_model_name_or_path}" | sed 's|.*/||')
  local generated_run_name="${model_slug}_${_muon_backend}_exp${exponent_clean}_lr${_learning_rate}"
